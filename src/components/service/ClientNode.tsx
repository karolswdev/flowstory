...
export const ClientNode = memo(function ClientNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--client shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, y: -15, scale: 0.85 } : { opacity: 0, scale: 0.9 }}
        animate={{
