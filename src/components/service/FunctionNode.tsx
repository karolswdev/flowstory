...
export const FunctionNode = memo(function FunctionNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--function shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, x: 20, skewX: -15 } : { opacity: 0, scale: 0.9 }}
        animate={{
