...
export const FirewallNode = memo(function FirewallNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--firewall shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scale: 0.5, rotate: 22.5 } : { opacity: 0, scale: 0.9 }}
        animate={{
          opacity: 1, scale: 1, rotate: 0,
          filter: isActive ? 'brightness(1.05)' : 'brightness(1)',
        }}
        transition={isNew ? { type: 'spring', stiffness: 350, damping: 22, mass: 0.8 } : { duration: 0.2 }}
      >
        <NodeHandles />
