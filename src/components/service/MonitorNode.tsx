...
export const MonitorNode = memo(function MonitorNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--monitor shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, y: 10, scale: 0.85 } : { opacity: 0, scale: 0.9 }}
        animate={{
      ...
      >
        <NodeHandles />
        <div className="shape-node__chart-accent" />
