...
export const PolicyNode = memo(function PolicyNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--policy shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scale: 0.5, rotate: 10 } : { opacity: 0, scale: 0.9 }}
