...
export const DomainEventNode = memo(function DomainEventNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--domain-event shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, y: -20 } : { opacity: 0, scale: 0.9 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={isNew ? { type: 'spring', stiffness: 400, damping: 22 } : { duration: 0.2 }}
      >
        <NodeHandles />
...
export default DomainEventNode;
