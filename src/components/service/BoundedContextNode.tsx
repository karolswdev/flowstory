...
export const BoundedContextNode = memo(function BoundedContextNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--bounded-context shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scale: 0.8, x: 0 } : { opacity: 0, scale: 0.9 }}
        animate={{ opacity: 1, scale: 1, x: 0 }}
        transition={isNew ? { type: 'spring', stiffness: 280, damping: 28 } : { duration: 0.2 }}
      >
        <NodeHandles />
...
export default BoundedContextNode;
