...
export const EntityNode = memo(function EntityNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--entity shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scale: 0.5, y: 20 } : { opacity: 0, scale: 0.9 }}
