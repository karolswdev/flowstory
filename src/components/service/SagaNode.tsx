...
export const SagaNode = memo(function SagaNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--saga shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, x: -30 } : { opacity: 0, scale: 0.9 }}
        animate={{ opacity: 1, x: 0, scale: 1 }}
        transition={isNew ? { type: 'spring', stiffness: 350, damping: 20 } : { duration: 0.2 }}
      >
        <NodeHandles />
