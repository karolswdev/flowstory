...
export const HumanTaskNode = memo(function HumanTaskNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--human-task shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, y: -12, scale: 0.8 } : { opacity: 0, scale: 0.9 }}
        animate={{
      ...
      >
        <NodeHandles />
        <span className="shape-node__person-badge">
