...
export const RepositoryNode = memo(function RepositoryNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--repository shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scale: 0.6, y: 15 } : { opacity: 0, scale: 0.9 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={isNew ? { type: 'spring', stiffness: 300, damping: 22 } : { duration: 0.2 }}
      >
        <NodeHandles />
