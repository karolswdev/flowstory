...
export const WorkerNode = memo(function WorkerNode({ data }: WorkerNodeProps) {
  const { name, type, technology, isActive = false, isComplete = false, isNew = false } = data;
  const stateClass = isActive ? 'active' : isComplete ? 'complete' : 'inactive';
  const icon = type === 'event-processor' ? '⚡' : '👷';

  return (
    <motion.div
      className={`shape-node shape-node--worker shape-node--${stateClass}`}
    ...
    >
      ...
      <div className="shape-node__body">
        <span className="shape-node__icon">{icon}</span>
