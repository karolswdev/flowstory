...
export const LoadBalancerNode = memo(function LoadBalancerNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--load-balancer shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scaleX: 0.4, scaleY: 0.8 } : { opacity: 0, scale: 0.9 }}
        animate={{
          opacity: 1, scaleX: 1, scaleY: 1, scale: 1,
