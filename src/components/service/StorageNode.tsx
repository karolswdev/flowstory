...
export const StorageNode = memo(function StorageNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--storage shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, scaleY: 0.3, y: 15 } : { opacity: 0, scale: 0.9 }}
        animate={{
          opacity: 1, scaleY: 1, scale: 1, y: 0,
