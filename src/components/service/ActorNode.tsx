...
export const ActorNode = memo(function ActorNode({ data }: { data: ServiceNodeData }) {
  ...
  return (
    <EffectWrapper nodeId={id} effects={effects} isActive={isActive} isRevealed={!isNew || isActive || isComplete}>
      <motion.div
        className={`shape-node shape-node--actor shape-node--${stateClass} ${failureClass}`}
        initial={isNew ? { opacity: 0, y: 25, scale: 0.8 } : { opacity: 0, scale: 0.9 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={isNew ? { type: 'spring', stiffness: 400, damping: 18, mass: 0.8 } : { duration: 0.2 }}
      >
        <NodeHandles />
